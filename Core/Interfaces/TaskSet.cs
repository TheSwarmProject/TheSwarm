...
public class TaskSet
{
    ...
    public TaskSet(TaskExecutor executor)
    {
        ...
        List<PropertyInfo> clientProps = this.taskSetInstance.GetType().GetRuntimeProperties()
            .Where(p => p.GetCustomAttributes(typeof(RegisterSwarmClient), false).Length > 0)
            .ToList();
        // Given the fact that user can use multiple clients - we process all occurrences of the attribute
